...
export async function requireMarketplacePricingManager(returnTo: string) {
  const client = await createClient();
  const {
    data: { user },
  } = await client.auth.getUser();
  ...
  const { data } = await client.rpc("can_manage_marketplace_pricing");
  if (data !== true) forbidden();
  return { client, user };
}

export async function requireMarketplaceOrdersManager(returnTo: string) {
  const client = await createClient();
  const {
    data: { user },
  } = await client.auth.getUser();
  ...
  const { data } = await client.rpc("can_manage_marketplace_orders");
  if (data !== true) forbidden();
  return { client, user };
}

export async function requireMarketplacePayablesManager(returnTo: string) {
  const client = await createClient();
  const {
    data: { user },
  } = await client.auth.getUser();
  ...
  const { data } = await client.rpc("can_manage_marketplace_payables");
  if (data !== true) forbidden();
  return { client, user };
}

export async function requireMarketplaceClaimsManager(returnTo: string) {
  const client = await createClient();
  const {
    data: { user },
  } = await client.auth.getUser();
  ...
  const { data } = await client.rpc("can_manage_marketplace_claims");
  if (data !== true) forbidden();
  return { client, user };
}
