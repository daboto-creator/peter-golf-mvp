...
class OpenAICompatibleProvider implements BestRoundConversationProvider {
  ...
  async interpretTurn(
    ...
    const semanticReasonInstruction = " reasonMode debe ser CATALOG_REASON cuando preguntan por qué apareció/muestraste el producto, y PERSONAL_FIT_REASON cuando preguntan por qué les conviene o es para ellos. Incluye siempre reasonMode en el JSON. El contrato exige catalogScopeIntent: EXPLICIT_FAMILIES|INHERIT_PREVIOUS_FAMILY|ALL_CLUBS|ALL_HANDED_EQUIPMENT|ALL_EQUIPMENT|null, requestedProductFamilies: array de DRIVER|FAIRWAY_WOOD|HYBRID|IRON|WEDGE|PUTTER|SET (vacío si el alcance es amplio), searchScopeMode: EXACT|MULTI_FAMILY|ALL_CLUBS|ALL_EQUIPMENT|null, searchContinuationRelation: KEEP_SCOPE|BROADEN_SCOPE|REPLACE_SCOPE|null y searchContinuationReason: EXPLICIT_CURRENT_TURN|ELLIPTICAL_CONTINUATION|PREVIOUS_SCOPE_EXHAUSTED|null. 'qué tienes para zurdo', 'qué tienes disponible para zurdo' y 'qué opciones tienes para alguien zurdo' son CATALOG_SEARCH con catalogScopeIntent=ALL_HANDED_EQUIPMENT, aunque antes se hablara de SET. 'otro bastón para zurdo' usa ALL_CLUBS y excluye SET. 'tienes un driver o un wedge?' y 'muéstrame drivers o wedges' son CATALOG_SEARCH con catalogScopeIntent=EXPLICIT_FAMILIES, requestedProductFamilies=[DRIVER,WEDGE] y MULTI_FAMILY; sólo preguntas que pidan comparar o cuál conviene son ASK_COMPARISON. Una búsqueda explícita reemplaza el alcance anterior, pero conserva hechos del jugador.";
