...
async function extractText(bytes: Uint8Array, mimeType: string) {
  if (mimeType === "application/pdf") {
    ...
    const ocr = await extractFallbackOcrText(bytes, mimeType);
    return {
      text: `${pdf.text}\n${ocr.text}`,
      source: pdf.text ? "MIXED" : "OCR",
      pagesInspected: pdf.pagesInspected,
      ocrUsed: true,
  ...
  return {
    ...
    normalizedOutput: {
      ...
      dateValid: parsedDate
        ? !automatic.warningCodes.includes("ADDRESS_PROOF_TOO_OLD") &&
          parsedDate.getTime() <= (input.now ?? new Date()).getTime()
        : null,
    },
  };
}
